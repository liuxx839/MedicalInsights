...
# New imports for object detection and similarity search
...
# New functions for object detection and similarity search
...
def process_prediction(image, results, selected_classes, conf_threshold, box_color):
    img = np.array(image)
...
def detect_face(image, face_detector, conf_threshold, face_box_color):
    img = np.array(image)
    img_rgb = img.copy()
            ...
            messages=[
                {
                    ...
                    参会人露脸识别：是否正确识别人物透露脸情况（全部露脸 / 部分未露脸 / 全部未露脸）？
...
def setup_object_detection():
    ...
    try:
                ...
                with st.spinner(f"正在使用 {model_option} 进行物体检测..."):
                    ...
                    if detect_faces:
                        with st.spinner("正在进行人脸检测..."):
                            _, detected_faces = detect_face(original_image_for_face, face_detector, face_confidence, face_box_color)
                            st.session_state.detected_faces = detected_faces
                    processed_img = process_prediction(image, results, selected_classes, confidence, box_color)
                    if detect_faces:
        ...
        if "未能下载预训练权重" in str(e) or "Failed to download" in str(e):
            st.warning(f"首次运行时需要下载YOLO模型，请确保您有稳定的网络连接。较大的模型下载可能需要更长时间。")
    ...
    with st.expander("使用说明"):
        ...
        ### 模型比较：
        - YOLOv11x/YOLOv8x: 最高精度，但处理速度较慢，适合对精度要求高的场景
        - YOLOv11l/YOLOv8l: 高精度，速度适中
        - YOLOv11m/YOLOv8m: 平衡的精度和速度
        - YOLOv11s/YOLOv8s: 较快的速度，精度适中
        - YOLOv11n/YOLOv8n: 最快的速度，但精度较低
...
def main():
    ...
    # Updated page selection with new option
